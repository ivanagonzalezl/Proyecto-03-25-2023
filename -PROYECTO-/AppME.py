    ...
    def from_txt_to_list(self):
        ...
        with open(directory+file_name) as f:
            for line in f:
                ...
                cliente_vip= Vip(name, dni, age, race, ondulado, n_ticket, validado, 0)
        ...
        for race in self.appMa.races:
            for i, carrera in carreras.items():
                if carrera.get("circuito") == None:
                    if race.race_name==i:
                        carreras[i]["circuito"]=str(race.race_circuit)
